...
const LockScreen = ({ onUnlock, isAutoLock = false, onCancel }: LockScreenProps) => {
  const [password, setPassword] = useState("");
  const [showPassword, setShowPassword] = useState(false);
  const [error, setError] = useState("");
  const [isShaking, setIsShaking] = useState(false);
  const [ownerPw, setOwnerPw] = useState("twowheels2024");

  useEffect(() => {
    supabase.from("workshop_settings").select("value").eq("key", "owner_password").single()
      .then(({ data }) => { if (data?.value) setOwnerPw(data.value); });
  }, []);
        ...
        >
          ...
          <div className="mb-6 flex flex-col items-center">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Admin Access — Enter owner password
            </p>
          </div>
